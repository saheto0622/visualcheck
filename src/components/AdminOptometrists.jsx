...
export default function AdminOptometrists({ onBack }) {
  ...
  async function crear() {
    const n = nombre.trim(), c = codigo.trim();
    if (!n || !c) return;
    setSaving(true);
    setError("");
    try {
      const record = { id: Date.now(), nombre: n, codigo: c, activo: true };
      ...
      {optometristas.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {optometristas.map(opt => (
            <div key={opt.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", padding: "12px 14px", background: "var(--color-background-primary)" }}>
              <div>
